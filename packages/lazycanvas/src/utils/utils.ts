  ...
  if (v instanceof Link) {
    ...
    const additionalSpacing =
      parserInstance.parse(
        v.additionalSpacing,
        defaultArg.wh(layer.width, layer.height),
        defaultArg.vl(options.vertical, options.layer),
      ) || 0;
...
function getLayerWidth(
  ...
  if (
    anyLayer instanceof LineLayer ||
    anyLayer instanceof BezierLayer ||
...
function getLayerHeight(
  ...
  return anyLayer instanceof Path2DLayer
    ? 0
...
export function parser(ctx: SKRSContext2D, canvas: Canvas | SvgCanvas, manager?: LayersManager) {
  return {
    parse(
      v: ScaleType,
      layer: { width: number; height: number } = defaultArg.wh(),
      options: { vertical?: boolean; layer?: boolean } = defaultArg.vl(),
    ) {
    ...
    parseBatch(
      values: Record<
        string,
        {
          ...
          options?: { vertical?: boolean; layer?: boolean };
        }
      >,
    ) {
      ...
      for (const key in values) {
        ...
        result[key] = parseToNormal(
          v,
          ctx,
          canvas,
          layer ?? defaultArg.wh(),
          options ?? defaultArg.vl(),
          manager,
        );
      }
      return result;
...
export function transform(
  ...
  extra: { text: string; textAlign: AnyTextAlign; fontSize: number; multiline: boolean } = {
    text: "",
  ...
  if (transform) {
    ...
    if (transform.matrix) {
      if (
        ...
        throw new LazyError("The matrix transformation must be a valid matrix");
      ctx.transform(
        transform.matrix.a,
        transform.matrix.b,
        transform.matrix.c,
      ...
      if (!(layer instanceof Div || layer instanceof Path2DLayer)) {
        ...
        if ("transform" in layer.props && layer.props.transform) {
          if (layer.props.transform.translate) {
            layer.props.transform.translate.x = resize(
              layer.props.transform.translate.x,
              ratio,
            ) as number;
            layer.props.transform.translate.y = resize(
              layer.props.transform.translate.y,
              ratio,
            ) as number;
          }
