...
const canvas = new LazyCanvas({ debug: true }).create(800, 780);
...
canvas.manager.layers.add(
  ...
  ...[
    ...
    const layers = (c: { centring: Centring; offset: { x: number; y: number } }, ind: number) => {
      return new Div().setID(`gradient-test-${ind}`).add(
  ...
  ...[0, 45, 90, 135, 180, 225, 270, 315].flatMap((angle) => {
    return [
      new TextLayer()
        .setPosition(
          new Link().setSource(`gradient${angle}deg9`).setType("x").setSpacing(-30),
          `link-y-gradient${angle}deg9-40`,
        )
        .setColor("#000")
        .setAlign("center")
        .setText(`${angle}°`)
