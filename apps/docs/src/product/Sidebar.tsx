...
const NavigationItemComponent: React.FC<{
  ...
  // Recursive function to check if any nested child matches the current path
  const hasActiveDescendant = (items: NavigationItem[]): boolean => {
    return items.some(child => {
      const childSlug = child.slug;
      const childSegments = childSlug.split('/').filter(Boolean);

      // Check direct match first
      const isDirectMatch = pathname === `/${childSlug}` || pathname.startsWith(`/${childSlug}/`);
      ...
      // If this child has nested children, check them recursively
      const hasNestedMatch = child.children ? hasActiveDescendant(child.children) : false;

      return isDirectMatch || isPathMatch || hasNestedMatch;
    });
  };

  // For accordion sections, check if any child's path is in the current URL (including nested)
  const hasActiveChild = item.children ? hasActiveDescendant(item.children) : false;
